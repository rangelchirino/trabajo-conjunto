...
namespace sdcv {
	...
	Track::Track(sdcv::Blob newBlob) {
		// TRACKING VARIABLES
		id = 0;
		bbox.push_back(newBlob.getBBox());						//
		...
		lastVisibleFrame = 1;									// Get current frame
		...
		OcclusionRes = newBlob.getOccluded();					//
	...
	Track::Track(sdcv::Blob newBlob, int NbFrame) {
		id = 0;
		ObjectID = 0;											// New object added ID
		...
		estimatedArea = 0;
		consecutiveBackwardDir = 0;
		lastVisibleFrame = NbFrame;		// Get current frame
		totalFrames = 1;
		totalVisibleFrames = 1;
		consInvisibleFrames = 0;
		OcclusionRes = newBlob.getOccluded();
